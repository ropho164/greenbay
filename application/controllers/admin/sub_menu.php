...
class Sub_menu extends CI_Controller {
	...
	public function index()
	{
		...
		$config["total_rows"] = $this->M_sub_menu->record_count();
		...
		$data["results"] = $this->M_sub_menu->listData($config["per_page"], $page);
		
		$data["links"] = $this->pagination->create_links();
		
		$data['count'] = $this->M_sub_menu->record_count();
		$this->load->view("admin/submenu/list", $data);
	}
	public function delrecord()
	{
		$session_data = $this->session->userdata('logged_in');
		$data = $this->M_sub_menu->delData($this->input->post('id'));
	...
	public function delmultirecord()
	{
		...
		if($this->input->post('delete'))
		{
			...
			{
				$data = $this->M_sub_menu->delData($checkbox[$i]);
				if($data==0){
					break;	
				}
			}
	...
	public function proaddata(){
		...
		$this->form_validation->set_rules('main_menu_id', 'lang:not_blank', 'trim|required|numeric');
		$this->form_validation->set_rules('sub_name_en', 'lang:not_blank', 'trim|required');
		$this->form_validation->set_rules('sub_name_vn', 'lang:not_blank', 'trim|required');
		$this->form_validation->set_rules('sub_order', 'lang:category_order', 'trim|required|numeric');
		...
		if($this->form_validation->run() == TRUE){
			$linkEN = mb_strtolower(url_title(removesign($this->input->post('sub_name_en'))));
			$cnt_link_en = $this->M_sub_menu->checkLinkEN(0,$linkEN);
			if($cnt_link_en>0){
				$linkEN = $linkEN.'-'.$cnt_link_en;
			}
			$linkVN = mb_strtolower(url_title(removesign($this->input->post('sub_name_vn'))));
			$cnt_link_vn = $this->M_sub_menu->checkLinkVN(0,$linkVN);
			if($cnt_link_en>0){
				$linkVN = $linkVN.'-'.$cnt_link_vn;
			}
			$data_ins = array(
				'uid'=>mysql_real_escape_string($session_data['id']),
				'main_menu_id'=>$this->input->post('main_menu_id'),
				'sub_order'=>removeDauNhayKep($this->input->post('sub_order')),
				'sub_name_en'=>removeDauNhayKep($this->input->post('sub_name_en')),
				'sub_name_vn'=>removeDauNhayKep($this->input->post('sub_name_vn')),
				'sub_lnk'=>mb_strtolower(url_title(removesign($this->input->post('sub_name_en')))),
				'sub_lnk_en'=>$linkEN,
				'sub_lnk_vn'=>$linkVN,
			...
			$b_Check_Ins = $this->M_sub_menu->insData($data_ins);
			$b_Check = true;
		}
		$data['b_Check']= $b_Check;
		$data['b_Check_Ins']= $b_Check_Ins;
		if($b_Check && $b_Check_Ins>0){
			redirect('./admin/sub_menu', 'refresh');
		}
		else{
			$data['results_main_menu'] = $this->M_main_menu->listAllDataActive(1);
			$this->load->view("admin/submenu/add", $data);	
	...
	public function editdata(){
		...
		$data["results_data_edit"] = $this->M_sub_menu->getDataEdit($id);
	...
	public function proeditdata(){
		...
		if($this->form_validation->run() == TRUE){
			$linkEN = mb_strtolower(url_title(removesign($this->input->post('sub_name_en'))));
			$cnt_link_en = $this->M_sub_menu->checkLinkEN($id,$linkEN);
			if($cnt_link_en>0){
				$linkEN = $linkEN.'-'.$cnt_link_en;
			}
			$linkVN = mb_strtolower(url_title(removesign($this->input->post('sub_name_vn'))));
			$cnt_link_vn = $this->M_sub_menu->checkLinkVN($id,$linkVN);
			if($cnt_link_en>0){
				$linkVN = $linkVN.'-'.$cnt_link_vn;
			}
			$data_upd = array(
			...
			if($this->input->post('idget')>0 && $this->input->post('idget')){ 
				$b_Check_Ins = $this->M_sub_menu->updateData($this->input->post("idget"),$data_upd);
			}
			$b_Check = true;
		}
		$data['b_Check']= $b_Check;
	...
	public function updOrder(){
		...
		$data_upd = array();
		if($this->form_validation->run() == TRUE){
			$data_upd = array('sub_order'=>$this->input->post('category_order'));
			if($this->input->post('id')>0 && $this->input->post('id')){
				$data = $this->M_sub_menu->updateData($this->input->post("id"),$data_upd);
			}
			else{
				$data = -1;				
			}
