...
class C_booking extends CI_Controller {
	...
	public function index()
	{
		...
		$config["base_url"] = base_url() . "/admin/c_booking/index/";
		$config["total_rows"] = $this->M_booking->record_count();
		...
		$data["results"] = $this->M_booking->listData($config["per_page"], $page);
		
		$data["links"] = $this->pagination->create_links();
		
		$data['count'] = $this->M_booking->record_count();
		$this->load->view("admin/v_booking/list", $data);
	}
	public function delrecord()
	{
		$session_data = $this->session->userdata('logged_in');
		$data = $this->M_booking->delData($this->input->post('id'));
	...
	public function delmultirecord()
	{
		...
		if($this->input->post('delete'))
		{
			...
			{
				$data = $this->M_booking->delData($checkbox[$i]);
				if($data==0){
					break;	
				}
			}
	...
	public function editdata(){
		if($this->session->userdata('logged_in')){			
			...
			$data["results_data_edit"] = $this->M_booking->getDataEdit($id);
			$this->load->view("admin/v_booking/edit", $data);
		}
		else
		{
		  redirect('./admin/login', 'refresh');
	...
	public function proeditdata(){
		...
		if($this->session->userdata('logged_in')){
			...
			$this->form_validation->set_rules('booking_status', 'lang:lang_orther_num', 'numeric');
			$b_Check = false;
			$b_Check_Ins = 0;
			if($this->form_validation->run() == TRUE){
				...
				if($this->input->post('idget')>0 && $this->input->post('idget')){
					...
					$b_Check_Ins = $this->db->update("cms_reservation",$data_upd);
				}
				$b_Check = true;
			}
			...
			if($b_Check && $b_Check_Ins==1){
				redirect('./admin/c_booking', 'refresh');
			}
			else{
				$data["results_data_edit"] = $this->M_booking->getDataEdit($this->input->post('idget'));
				$this->load->view("admin/v_booking/edit", $data);	
			}
		}
		else
	...
	public function ansdata(){
		...
		if($this->session->userdata('logged_in')){			
			$id = ($this->uri->segment($this->config->item('uri_segment_id'))) ? $this->uri->segment($this->config->item('uri_segment_id')) : 0;
			$data["results_data_edit"] = $this->M_booking->getDataEdit($id);
			$this->load->view("admin/v_booking/ans", $data);
		}
		else
		{
		  redirect('./admin/login', 'refresh');
